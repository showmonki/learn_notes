...
out_src = os.listdir('./src')
FLOOR_IMGS = ['./src/'+ pic for pic in out_src]
BACK_IMGS = ['Heart', 'Dolphin', 'Turtle', 'Sheep']
...
class Player(object):
	def __init__(self, x, y, w, h):
		self.bounds = Rect(x, y, w, h)
		img = Image.open('./img/Sheep3.JPG').transpose(Image.FLIP_LEFT_RIGHT)  #Octopus
...
class FloorSprite(object):	
	...
	def set_random_bounds(self):
		...
		y = random.randint(env.playfield.min_y, env.ground_height)
		x = random.randint(env.playfield.min_x, env.playfield.max_x + env.playfield.w)
...
class BackgroundSprite(object):
	...
	def set_random_bounds(self):
		...
		y = random.randint(env.ground_height, env.playfield.max_y - size)
		if self.velocity < 0:
...
class Game(object):
	...
	def move_bubbles(self, dt):
		...
		for bubble in self.bubbles:
			if (bubble.bounds.min_y > self.env.playfield.max_y) or (bubble.bounds.min_x < self.env.playfield.min_x):
				x = random.randint(self.env.playfield.min_x, self.env.playfield.max_x + self.env.playfield.w)
				y = self.env.playfield.min_y - random.randint(0, self.env.bubble_max)
	...
	def move_floor_sprites(self, dt):
		if(self.env.state == GAME_READY) or (self.env.state == GAME_PLAY):
			...
			for sprite in self.floor_sprites:
				...
				if sprite.bounds.max_x < self.env.playfield.min_x:
					sprite.set_random_image()
					sprite.set_random_bounds()
					sprite.bounds.x = random.randint(self.env.playfield.max_x, self.env.playfield.max_x + self.env.playfield.w)
